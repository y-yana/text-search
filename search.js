  ...
  $.getJSON("data.json", function (data) {
    for (var i = 0; i < Object.keys(data.bookData).length; i++) {
      if (searchWord == data.bookData[i].title || searchWord == data.bookData[i].author || searchWord == data.bookData[i].type || searchWord == data.bookData[i].period || data.bookData[i].summary.match(searchWord)) {
        ...
        p.innerHTML ="<span id='title'>"+ data.bookData[i].title + "</span><br><br><span id='author'>著者：" + data.bookData[i].author + "</span><span id='type'>ジャンル：" + data.bookData[i].type + "</span><span id='period'>年代：" + data.bookData[i].period + "</span><br><br><span id='summary'>" + data.bookData[i].summary + "</span>";
